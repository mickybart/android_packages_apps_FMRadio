 ...
 * @hide
 */
class FmReceiverImpl extends FmReceiver {
    ...
    private onServiceAvailableListener mServiceAvailableCb;

    private ServiceConnection mConnection = new ServiceConnection() {
    	public void onServiceConnected(ComponentName className, IBinder service) {
    		Log.d(TAG, "onServiceConnected() name = " + className + ", service = " + service);
    		try {
    			mService = (IFmReceiver) IFmReceiver.Stub.asInterface(service);
    		} catch (Throwable t) {
    			mService = null;
    		}
    		if (mService == null) {
    			Log.e(TAG, "Unable to create service");
    		} else if (mServiceAvailableCb != null) {
    			mServiceAvailableCb.onServiceAvailable();
    		}
    	}
    ...
    FmReceiverImpl(Context ctx, onServiceAvailableListener cb) {
    	mContext = ctx;
    	mServiceAvailableCb = cb;

        Intent receiverIntent = new Intent(IFmReceiver.class.getName());
        ComponentName component = new ComponentName("com.android.bluetooth", "com.broadcom.fm.FmReceiverService");
	receiverIntent.setComponent(component);

	if (!mContext.bindService(new Intent(receiverIntent),
                              mConnection, Context.BIND_AUTO_CREATE)) {
			Log.e(TAG, "Could not bind to IFmReceiver Service");
        }
    }
